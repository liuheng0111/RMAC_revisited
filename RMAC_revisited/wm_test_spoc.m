...
% M Wang et al, RMAC Revisited...

% For oxford5k dataset 
% (mAP of our approach)
% mAP, without re-ranking = 0.7204
% mAP, after re-ranking = 0.7825
% mAP, after re-ranking and QE = 0.8185
...
mex aml_all_spoc.c; 									 % compile MEX for localization
...
if rerank
	...
	if ~use_rmac
        ...
        qvecs_loc_spoc = cellfun(@(x) vecpostproc(spoc(x, net)), qim, 'un', 0);
	end
end
...
if rerank
	for q = 1:numel(qim)
       ...
       bestbox = cellfun(@(x) aml_all_spoc(double(x), int32(10), double(qvecs_loc_spoc{q}), qratio, qratio_t, step_box, rf_step, rf_iter), conv3d_toplist, 'un', 0);        
       
	   % get MAC or R-MAC from the localized windows
	   if use_rmac		   
            vecs_bestbox = cellfun(@(x, b) vecpostproc(rmac_regionvec_act(x(b(8):b(9), b(10):b(11), :), L)), conv3d_toplist, bestbox, 'un', 0);                    
            vecs_bestbox = cellfun(@(y) vecpostproc(sum(vecpostproc(apply_whiten(y, Xm, eigvec, eigval)), 2)), vecs_bestbox, 'un', 0);
       else	
            vecs_bestbox = cellfun(@(x, b) mac_act(x(b(8):b(9), b(10):b(11), :)), conv3d_toplist, bestbox, 'un', 0);
...
% Elapsed time is 1085.149103 seconds. (retrieval part)